...
def clean_code(df1):
    ...
    Tipos de limpeza:
    1- Conversão da coluna Order_Date para o tipo datetime
    2- Exclusão da coluna Unnamed: 0
    3- Remoção dos dados NaN, e dos espaços da coluna Festival
    4- Criação e convesão da coluna week_of_year
    5- Remoção do texto '(min) ' da coluna Time_taken(min)
    6- Conversão da coluna Time_taken(min) para int
    input: dataframe
    output: dataframe
...
def traffic_order_share(df1):
    # #Gráfico pizza
    df_aux3 = (df1.loc[:, ['ID', 'Road_traffic_density']]
