...
def distance(df1, fig):
    if fig == False:
        cols = (['Delivery_location_latitude', 'Restaurant_latitude',
...
def avg_std_time_on_traffic(df1):
    df_aux2 = (df1.loc[:, ['City', 'Time_taken(min)', 'Road_traffic_density']]
...
with tab1:
    ...
    with st.container():
        ...
        with col3:
            # st.markdown('###### Tempo entrega médio c/ Festival')
            df_aux = avg_std_time_delivery(df1, 'Yes', 'avg_time')
            col3.metric('Tempo médio c/ Festival',df_aux)


        with col4:
            # st.markdown('###### Desvio Padrão de entregas médio c/ festival')
        ...
        with col6:
            # st.markdown('###### Desvio Padrão de entregas médio s/ festival')
